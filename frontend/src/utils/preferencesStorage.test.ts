...
describe("mergeGraphSettings", () => {
  ...
  it("discards prefs saved under a different crop", () => {
    const prefs = buildFieldPrefs({ ...defaults(), forecastDays: 7 }, { gddChartMode: "daily", etChartMode: "precip" }, "almond");
  ...
  it("restores saved presentation prefs over defaults key-by-key", () => {
    ...
    const prefs = buildFieldPrefs(saved, { gddChartMode: "daily", etChartMode: "referenceEt" }, "tomato");
  ...
  it("drops comparison years at or beyond the current year", () => {
    ...
    const prefs = buildFieldPrefs(saved, { gddChartMode: "cumulative", etChartMode: "cropEt" }, "tomato");
    const merged = mergeGraphSettings(defaults(), prefs, CONTEXT);
  ...
  it("ignores a start date saved in a prior season", () => {
    const prefs = buildFieldPrefs({ ...defaults(), startDate: "2025-03-01" }, { gddChartMode: "cumulative", etChartMode: "cropEt" }, "tomato");
  ...
  it("preserves newly added default flags absent from older saved prefs", () => {
    const prefs = buildFieldPrefs(defaults(), { gddChartMode: "cumulative", etChartMode: "cropEt" }, "tomato");
...
describe("localPreferencesRepository", () => {
  ...
  it("round-trips and clears per-field prefs", () => {
    const prefs = buildFieldPrefs({ ...defaults(), forecastDays: 14 }, { gddChartMode: "daily", etChartMode: "precip" }, "tomato");
  ...
  it("keeps other fields' prefs when clearing one", () => {
    localPreferencesRepository.save("field-a", buildFieldPrefs(defaults(), { gddChartMode: "daily", etChartMode: "cropEt" }, "tomato"));
    localPreferencesRepository.save("field-b", buildFieldPrefs(defaults(), { gddChartMode: "cumulative", etChartMode: "precip" }, "almond"));
    localPreferencesRepository.clear("field-a");
